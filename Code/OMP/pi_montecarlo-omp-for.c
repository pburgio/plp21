...
int main()
{
	...
	#pragma omp parallel for reduction(+:in_count, tot_count) num_threads(N_THREADS) private(counter)
	{
		for(counter=0; counter<tot_iterations; counter++)
		{
			_printf("[Thread %d/%d] I am executing iteration %llu\n", omp_get_thread_num(), omp_get_num_threads(), counter);
			float x = randNumGen();
			float y = randNumGen();
